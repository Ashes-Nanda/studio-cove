    ...
    <div className="relative flex flex-col items-center justify-center py-32 bg-ink font-sans text-white">
      {heading && (
        <div className="w-full max-w-7xl px-6 mb-16">
          ...
          {subheading && (
            <p className="text-ink/40 mt-4 text-sm max-w-md">{subheading}</p>
          )}
        </div>
      )}
        ...
        {options.map((option, index) => (
          <div
            ...
            style={{
              ...
              backgroundSize: activeIndex === index ? (option.activeBackgroundSize ?? 'contain') : 'cover',
              backgroundRepeat: 'no-repeat',
              backgroundPosition: activeIndex === index ? (option.activeBackgroundPosition ?? 'center') : 'center',
              opacity: animatedOptions.includes(index) ? 1 : 0,
              transform: animatedOptions.includes(index) ? 'translateX(0)' : 'translateX(-60px)',
              minWidth: '60px',
              borderWidth: '2px',
              borderStyle: 'solid',
              borderColor: activeIndex === index ? 'rgba(255,255,255,0.3)' : 'rgba(255,255,255,0.06)',
              backgroundColor: '#111',
